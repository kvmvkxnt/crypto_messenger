...
class Transaction:
    ...
    def __init__(self, sender: str,
        ...
        if sender:
            self.sender_public_key = sender_public_key if isinstance(sender_public_key, rsa.RSAPublicKey) else load_pem_public_key(sender_public_key.encode('utf-8')) 
        else:
    ...
    def to_dict(self) -> Dict[str, str]:
        ...
        return {
            ...
            "amount": self.amount,
            "content": str(self.content) , # handling bytes
            "signature": base64.b64encode(self.signature).decode('utf-8') if self.signature else None,
            "sender_public_key": self.sender_public_key.public_bytes(Encoding.PEM, PublicFormat.SubjectPublicKeyInfo).decode('utf-8') if self.sender_public_key else None
         }
